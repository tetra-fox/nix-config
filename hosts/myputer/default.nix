# 2020 13" macbook pro (intel, MacBookPro16,2)
{
  ...
  homebrew = {
    ...
    onActivation = {
      # brew's cask definitions come from its json api, which refreshes on its
      # own whenever the cache goes stale, while the brew checkout only moves on
      # an explicit update. left off, the two drift apart until the api starts
      # emitting cask dsl the installed brew can't parse: that is what broke
      # keka, obs, xld and zen with "undefined method 'command_wrapper'", with
      # api data generated by 6.0.18 against a 6.0.11 checkout. updating keeps
      # the parser and the data in step. upgrade stays off, this only moves brew
      # itself, not the installed packages
      autoUpdate = true;
      upgrade = false;
      cleanup = "uninstall";
    };
