...
# so we run it ourselves below to keep the check.
{
  ...
  # the internal split-horizon zone, a real zone file (files/mesa.tetra.cool.zone.in) with the
  # two derived addresses substituted in: @nsIp@ = this resolver's IP, @edgeVip@ = the edge VIP
  # the wildcard answers from. the thing unbound couldn't express cleanly. unifi is a
  # more-specific record that wins over the wildcard, no special-casing needed.
  mesaZone = pkgs.replaceVars ./files/mesa.tetra.cool.zone.in {
