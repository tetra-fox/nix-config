...
# separate sites, so these facts MUST be per-site. the shared layout means the sites can
# never be routed or bridged together, see the note in mesa.nix; that constraint is
# accepted on purpose. the shared proxmox-VM network shape lives in _common.nix; fairlane
