{
  ...
  options.lab.steam.lighthouses = lib.mkOption {
    type = lib.types.listOf lib.types.str;
    default = [];
    description = "steamvr basestation ids (LHB-...) to power on with monado and stand by after; device facts, set per host. empty = no power-management unit";
  };
  ...
  config = {
    ...
    systemd.user.services.lighthouse-power-management = let
      basestations = lib.concatMapStrings (b: " -b ${b}") config.lab.steam.lighthouses;
    in
      lib.mkIf (config.lab.steam.lighthouses != []) {
        ...
        serviceConfig = {
          ...
          ExecStart = "${pkgs.lighthouse-steamvr}/bin/lighthouse -s on${basestations}";
          ExecStop = "${pkgs.lighthouse-steamvr}/bin/lighthouse -s standby${basestations}";
        };
