...
# prune any live item whose name isn't in the declared set
declared_names=$(jq '[.[].name]' "$ITEMS_FILE")
echo "$LIVE" | jq -c '.[]' | while IFS= read -r live_item; do
...
build_body() {
  local item="$1" base="$2"
  local top fields secret_field secret_file
  ...
  if [ -n "$secret_field" ]; then
    echo "$base" | jq \
      --argjson top "$top" --argjson f "$fields" \
...
# create-or-update each declared item
jq -c '.[]' "$ITEMS_FILE" | while IFS= read -r item; do
  ...
  if [ -n "$existing" ]; then
    ...
    build_body "$item" "$existing" | arr_send PUT "${BASE_URL}/${RESOURCE}/${id}" >/dev/null
    ...
    build_body "$item" "$schema" | arr_send POST "${BASE_URL}/${RESOURCE}" >/dev/null
    echo "  created ${name}"
  fi
done
