...
# module imports the same modules.services.postgres.options and reads cfg.roles, so the set of
...
in {
  ...
  config = lib.mkIf ha.enable {
    ...
    # modules.meta.vrrp.system (edge + dns use it too); this supplies the db-specific values. both the
